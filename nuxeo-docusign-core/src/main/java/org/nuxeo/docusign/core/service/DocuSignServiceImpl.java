...
public class DocuSignServiceImpl extends DefaultComponent implements DocuSignService {
    ...
    @Override
    public String send(CoreSession session, List<Blob> blobs, String subject, List<String> signerEmails,
            Map<String, String> customFields, String callbackUrl) throws Exception {
        ...
        List<Document> documents = new ArrayList<>();
        {
            ...
            for (Blob blob : blobs) {
                ...
                documents.add(new Document().name(blob.getFilename())
                                            .documentId("" + i++)
                                            .documentBase64(base64)
                                            .fileExtension(FilenameUtils.getExtension(blob.getFilename())));
            }
        }
        ...
        if (callbackUrl != null && callbackUrl.length() > 0) {
            ...
            notification.setEnvelopeEvents(Collections.singletonList(
                    new EnvelopeEvent().envelopeEventStatusCode("completed").includeDocuments("false")));
    ...
    @Override
    public String send(CoreSession session, DocumentModelList docs, String subject, List<String> signerEmails,
            Map<String, String> customFields, String callbackUrl) throws Exception {
    ...
    public DocumentModelList updateDocuments(CoreSession session, String envelopeId) throws Exception {
        ...
        PageProvider<DocumentModel> pp = (PageProvider<DocumentModel>) pps.getPageProvider("DocuSignEnvelopeDoc", null,
                50L, 0L, props, envelopeId);
    ...
    public void handleCallbackEvent(InputStream stream, String chainName) {
        ...
        if (chainName == null)
            chainName = config.getDefaultCallbackChain();
